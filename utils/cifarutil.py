...
class Cifar:
    ...
    def getweights(self, nm):
        print("layers",  len(nm.layers))
        self.WC1, self.bC1 = nm.layers[0].get_weights()
        print("SHAPE", self.WC1.shape, self.bC1.shape)
    def cifarviz(self, nm):
        pass
    def modifyThroughInterSection(self, model, x, y):
        pass
    ...
    def extract_sliding_windows_gradw(self, x,
                                      ksize,
                                      pad,
                                      stride,
                                      orig_size,
        ...
        h2 = orig_size[0]
        w2 = orig_size[1]
        ph = int(self.calc_pad(pad, h, h2, 1, ((kh - 1) * sh + 1)))
        pw = int(self.calc_pad(pad, w, w2, 1, ((kw - 1) * sw + 1)))
    ...
    def extract_sliding_windows_gradx(self,x,
                                      ksize,
                                      pad,
                                      stride,
                                      orig_size,
    ...
    def conv2d_gradw(self, x, dy, ksize, pad='SAME', stride=(1, 1)):
        ...
        ksize2 = dy.shape[:2]
        x = self.extract_sliding_windows_gradw(x, ksize2, pad, stride, ksize)
        dys = dy.shape
    ...
    def conv2d_gradx(self,w, dy, xsize, pad='SAME', stride=(1, 1)):
        ...
        if pad == 'SAME':
            dys = dy.shape[1:3]
            pad2h = int(
                self.calc_pad('SAME', max(dys[0], dys[0] * stride[0] - 1), xsize[0], 1,
                         ksize[0]))
            pad2w = int(
                self.calc_pad('SAME', max(dys[0], dys[0] * stride[1] - 1), xsize[1], 1,
                         ksize[1]))
            pad2 = (pad2h, pad2w)
        elif pad == 'VALID':
            pad2 = (int(self.calc_pad('SAME', 0, 0, 1, ksize[0])),
                    int(self.calc_pad('SAME', 0, 0, 1, ksize[1])))
            pad2 = (pad2[0] * 2, pad2[1] * 2)
        else:
            pad2 = pad
        w = np.transpose(w, [0, 1, 3, 2])
        ksize = w.shape[:2]
        dx = self.extract_sliding_windows_gradx(dy, ksize, pad2, stride, xsize)
        dxs = dx.shape
    ...
    def pooling(self,mat,ksize,method='max',pad=False):
